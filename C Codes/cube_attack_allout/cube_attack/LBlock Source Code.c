...
unsigned char recon[80][10] = {	{0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
...
void LBlockKeySchedule(unsigned char *key, unsigned int *subkey, int r)
{
	int i, j;
	...
	for (i = 2; i <r; i++)				// r is the number of rounds
	{	
		for (j = 0; j < 10; j++)
		{
			temp1[j] = temp[j];
...
int LBlockEncrypt(unsigned char *plain, unsigned char *cipher, unsigned char *key, int r)
{
	int i,w=0,rem=0;
	unsigned int XL, XR, temp, temp1;
	unsigned int Subkey[32];

	LBlockKeySchedule(key, Subkey, r);
...
void test(int ivpos[], int r)
{
	unsigned int dtob[128]={0};
	unsigned char iv[128][64]={0};
	unsigned char plain2[128][8]={0};
	...
	unsigned char key0[10]={0}, key1[10]={0},key12[10]={0};
	int j=0,k=0,t=0,row=0,col=0,v=0,T=50;		//T = Number of Linearity tests
	double len=0,base=2;
	unsigned int num=0x0,num2=0x0;
	unsigned char sum[8]={0},p0[8]={0},p1[8]={0},p2[8]={0},p12[8]={0},z[8]={0};
	int xx=0,n0[64]={0},n1[64]={0},count[64]={0},bit=0,counter=0;
	...
	{
		LBlockEncrypt(plain2[j], cipher, key, r);
	...
	for (t=0;t<T;t++)		//T = Number of Linearity tests
	{
		...
		for (j=0;j<len;j++)
		{
			LBlockEncrypt(plain2[j], cipher, key0, r);
			for (xx=0;xx<8;xx++)
			{
				sum[xx]^=cipher[xx];
			}
		...
		for (j=0;j<len;j++)
		{
			LBlockEncrypt(plain2[j], cipher, key1, r);
			for (xx=0;xx<8;xx++)
			{
				sum[xx]^=cipher[xx];
			}
		...
		for (j=0;j<len;j++)
		{
			LBlockEncrypt(plain2[j], cipher, key12, r);
			for (xx=0;xx<8;xx++)
			{
				sum[xx]^=cipher[xx];
			}
		...
		for (xx=0;xx<8;xx++)
		{
			for(bit=0;bit<8;bit++)
			{
				if (((p0[xx] & 0x80>>bit) ^ (p1[xx] & 0x80>>bit) ^ (p2[xx] & 0x80>>bit))!=(p12[xx] & 0x80>>bit))	//Linear
				{
					count[(xx*8)+bit]++;
				}
			}
	...
	if (t==T)
	{
		...
		for (j=0;j<80;j++)
		{
			...
			for (re=0;re<len;re++)
			{
				z=LBlockEncrypt(plain2[re], cipher, recon[j], r);
				sum^=z;
			}
			if (p0^sum==1)
				printf("k%d ",j+1);
