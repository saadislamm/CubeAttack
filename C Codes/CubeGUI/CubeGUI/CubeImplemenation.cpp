...
int LBlockEncrypt(unsigned char *plain, unsigned char *cipher, unsigned char *key, int r, int out)
{
	int i,w=0,rem=0;
...
{
	unsigned int dtob[32]={0};
	unsigned char iv[32][64]={0};
	unsigned char plain2[32][8]={0};
	...
	int sum=0,p0=0,p1=0,p2=0,p12=0,z=0,n0=0,n1=0,j=0,k=0,t=0,row=0,col=0,v=0,T=30;		//T = Number of Linearity tests
	double len=0,base=2;
	...
	{
		z=LBlockEncrypt(plain2[j], cipher, key, r, out);
		sum^=z;	
	}
	p0=sum;
	...
	for (t=0;t<T;t++)		//T = Number of Linearity tests
	{
		...
		for (j=0;j<len;j++)
		{
			z=LBlockEncrypt(plain2[j], cipher, key0, r, out);
			sum^=z;
		}
		p1=sum;
		...
		for (j=0;j<len;j++)
		{
			z=LBlockEncrypt(plain2[j], cipher, key1, r, out);
			sum^=z;
		}
		p2=sum;
		...
		for (j=0;j<len;j++)
		{
			z=LBlockEncrypt(plain2[j], cipher, key12, r, out);
			sum^=z;
		}
		p12=sum;
	...
	if (t==T)
	{
		...
		for (j=0;j<80;j++)
		{
			...
			for (re=0;re<len;re++)
			{
				z=LBlockEncrypt(plain2[re], cipher, recon[j], r, out);
				sum^=z;
			}
			if (p0^sum==1)
				printf("k%d ",j+1);
